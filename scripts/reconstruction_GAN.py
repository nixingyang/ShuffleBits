...
from reconstruction_utils import (DataSequence, GeneratePredictions,
                                  apply_decoder, apply_discriminator,
                                  flatten_loss_function, generate_feature_maps,
                                  get_feature_arrays,
                                  integrate_preprocess_input, save_predictions)
...
flags.DEFINE_string("feature_reconstruction_model_file_path",
                    os.path.expanduser("~/.keras/models/Complete_ResNet50.h5"),
                    "File path of the feature reconstruction model.")
flags.DEFINE_string(
    "feature_reconstruction_model_intermediate_layer_name", "conv2_block3_out",
...
flags.DEFINE_string(
    "feature_reconstruction_model_preprocess_input_mode", "caffe",
    "Mode of preprocess_input of the feature reconstruction model.")
flags.DEFINE_bool("use_spectral_normalization_in_discriminator", False,
                  "Use spectral normalization in discriminator.")
flags.DEFINE_string("comparison_mode_in_discriminator", "concatenation",
...
flags.DEFINE_float("feature_reconstruction_loss_weight", 1.0,
                   "Weight of the feature reconstruction loss.")
flags.DEFINE_float("total_variation_loss_weight", 0.0,
                   "Weight of the total variation loss.")
flags.DEFINE_float("discriminator_loss_weight", 1.0,
                   "Weight of the loss in discriminator.")
flags.DEFINE_float("learning_rate_of_G", 0.0001, "Learning rate of generator.")
flags.DEFINE_float("learning_rate_of_D", 0.0001,
...
flags.DEFINE_integer("valid_steps_per_epoch", 50,
                     "Number of steps per epoch for validation.")
flags.DEFINE_integer("prediction_num", 4, "Number of predictions per epoch.")
flags.DEFINE_integer("discriminator_steps", 1,
                     "Number of consecutive steps to train discriminator.")
flags.DEFINE_integer("generator_and_discriminator_steps", 1,
                     "Number of consecutive steps to train generator.")
flags.DEFINE_bool("use_different_batches", True,
...
flags.DEFINE_string(
    ...
    os.path.abspath(
        os.path.join(
            __file__,
            "../../output_{}".format(datetime.now().strftime("%Y_%m_%d")))),
    "Path to directory to output files.")
FLAGS = flags.FLAGS


def init_model(image_shape, feature_shape,
               ...
               comparison_mode_in_discriminator, pixel_loss_weight,
               feature_reconstruction_loss_weight, total_variation_loss_weight,
               discriminator_loss_weight, learning_rate_of_G,
               learning_rate_of_D):
    """
    ...
    decoder_output_tensor = apply_decoder(input_tensor=decoder_output_tensor,
                                          factor=factor,
                                          num=num)
    decoder_model = Model(inputs=[decoder_input_tensor],
    ...
    discriminator_model = Model(inputs=[
        discriminator_input_target_tensor, discriminator_input_condition_tensor
    ],
                                outputs=[discriminator_output_tensor],
                                name="discriminator")
    discriminator_model.compile(
        optimizer=Adam(learning_rate=learning_rate_of_D),
    ...
    feature_reconstruction_loss = K.mean(flatten_loss_function()(
        ground_truth_image_feature_tensor, reconstructed_image_feature_tensor))
    generator_and_discriminator_model.add_metric(feature_reconstruction_loss,
                                                 name="feature_reconstruction",
                                                 aggregation="mean")
    ...
    total_variation_loss = K.mean(
        tf.image.total_variation(reconstructed_image_tensor) / tf.dtypes.cast(
            K.prod(K.int_shape(reconstructed_image_tensor)[1:]), tf.float32))
    generator_and_discriminator_model.add_metric(total_variation_loss,
    ...
    return generator_and_discriminator_model, discriminator_model, visualization_model, decoder_model
...
def main(_):
    ...
    feature_reconstruction_model_file_path, feature_reconstruction_model_intermediate_layer_name, feature_reconstruction_model_preprocess_input_mode = \
        FLAGS.feature_reconstruction_model_file_path, FLAGS.feature_reconstruction_model_intermediate_layer_name, FLAGS.feature_reconstruction_model_preprocess_input_mode
    use_spectral_normalization_in_discriminator, comparison_mode_in_discriminator = FLAGS.use_spectral_normalization_in_discriminator, FLAGS.comparison_mode_in_discriminator
    pixel_loss_weight, feature_reconstruction_loss_weight, total_variation_loss_weight, discriminator_loss_weight = FLAGS.pixel_loss_weight, FLAGS.feature_reconstruction_loss_weight, FLAGS.total_variation_loss_weight, FLAGS.discriminator_loss_weight
    learning_rate_of_G, learning_rate_of_D, batch_size, epoch_num = FLAGS.learning_rate_of_G, FLAGS.learning_rate_of_D, FLAGS.batch_size, FLAGS.epoch_num
    train_steps_per_epoch, valid_steps_per_epoch, prediction_num = FLAGS.train_steps_per_epoch, FLAGS.valid_steps_per_epoch, FLAGS.prediction_num
    discriminator_steps, generator_and_discriminator_steps, use_different_batches = FLAGS.discriminator_steps, FLAGS.generator_and_discriminator_steps, FLAGS.use_different_batches
    ...
    output_folder_path = os.path.join(
        ...
        "{}_{}_{}_{}".format(pixel_loss_weight,
    ...
    train_accumulated_info_dataframe, query_accumulated_info_dataframe, gallery_accumulated_info_dataframe, _ = \
    ...
    train_feature_array, valid_feature_array = get_feature_arrays(  # pylint: disable=unbalanced-tuple-unpacking
    ...
    generator_and_discriminator_model, discriminator_model, visualization_model, decoder_model = init_model(
        image_shape=image_shape,
        feature_shape=train_feature_array.shape[1:],  # pylint: disable=unsubscriptable-object
        feature_reconstruction_model_file_path=
        feature_reconstruction_model_file_path,
        feature_reconstruction_model_intermediate_layer_name=
        feature_reconstruction_model_intermediate_layer_name,
    ...
    visualize_model(model=generator_and_discriminator_model,
    ...
    if os.path.isfile(generator_and_discriminator_model_file_path):
        ...
        save_predictions(model=decoder_model,
                         batch_size=batch_size,
                         image_file_path_array=valid_accumulated_info_dataframe[
                             "image_file_path"].values,
                         root_folder_path=root_folder_path,
                         feature_array=valid_feature_array,
                         output_folder_path=os.path.join(
                             output_folder_path, "complete_valid"))
    else:
        ...
        for epoch_index in range(epoch_num):
            ...
            for batch_index in range(len(train_generator)):
                ...
                ones_array = np.ones((len(groundtruth_array), 1),
                                     dtype=np.float32)
                ...
                if use_different_batches:
                    optimize_discriminator = np.remainder(
                        batch_index, total_steps) in range(discriminator_steps)
                ...
                if optimize_discriminator:
                    ...
                    discriminator_loss = discriminator_model.train_on_batch(
                        x=[
                            np.vstack((groundtruth_array,
                                       reconstructed_target_array)),
                ...
                if optimize_generator:
                    ...
                    generator_and_discriminator_loss = generator_and_discriminator_model.train_on_batch(
                        x=input_array, y=[groundtruth_array, ones_array])

                # Print losses
                print("Batch {}: D {}, G&D {}".format(
                    batch_index, discriminator_loss,
